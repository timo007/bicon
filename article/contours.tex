...
\begin{enumerate}
\item \label{enum:max_distance} Determine the maximum absolute difference,
$\Delta$, in either longitude or latitude between any two consecutive points on
the contour line being encoded.
\item \label{enum:spatial_increment} Compute the spatial increment:
$\delta = 2\Delta/127$.
\item Get the longitude and latitude of the first point on the contour
line, $(\lambda_1, \phi_1)$.
...
$\delta$, from Step~\ref{enum:spatial_increment}.
\item \label{enum:relative_locations3} Round the relative locations from
Step~\ref{enum:relative_locations2} to integer values.
\item \label{enum:offsets} Compute the offsets of each of the points from
Step~\ref{enum:relative_locations3} for the points immediately preceding them
...
Because of the way $\delta$ was computed at Step~\ref{enum:spatial_increment},
the offsets from the previous step are guaranteed to lie within the range
$[-127,127]$, and can be expressed as signed 8-bit integers. This concludes the
current contour record.
\end{enumerate}
\end{tcolorbox}
