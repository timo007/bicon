...
difficult~\citep{burton_cruz2020}. Compression is one method, in conjunction
with others, to help tackle this explosion of data.
...
cheaper communications methods, this problem is not as acute as in the past.
However, there are still users who are limited by slow or expensive
communication.
...
be encoded, not just mean sea level pressure isobars. As an example,
Figure~\ref{fig:BinCon} shows mean sea level pressure for the New Zealand
region plotted from NCEP (National Centers for Environmental Prediction) GFS
(Global Forecast System) \ang{0.25} spatial resolution data compressed using
the BinCon method\footnote{The New Zealand region in this article refers to the
region from \ang{142}\,E--\ang{164}\,W, \ang{54}\,S--\ang{27}\,S.}. The data file used to
make this plot is just \SI{1845}{\byte} in size.
...
gridded data that needs compressing. For example, Figure~\ref{fig:nzmslp}\,(a)
...
tolerances. The resulting data were then decompressed and gridded and compared
...
other emergency situations. For example, in January 2022 communications to the
...
satellite-based systems and other technologies. Alternative
...
at \url{https://github.com/timo007/bicon}. Readers are free to download,
